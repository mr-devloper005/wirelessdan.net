...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse bg-[var(--editable-border)]', className)} />
}

export function PageLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div
      className={cn('mx-auto w-full max-w-[var(--editable-container)] px-4 py-16 sm:px-6 lg:px-8', className)}
      aria-live="polite"
      aria-busy="true"
    >
      <p className="editable-mono text-[0.7rem] tracking-[0.22em] text-[var(--slot4-muted-text)]">{label}</p>
      <PulseBlock className="mt-8 h-16 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-4 h-6 w-2/3 max-w-2xl" />
      <div className="mt-12 grid gap-8 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="border border-[var(--editable-border)] p-6">
...
export function DetailLoadingState({ label = 'Loading detail', className }: LoadingStateProps) {
  return (
    <div
      className={cn(
        'mx-auto grid w-full max-w-[var(--editable-container)] gap-10 px-4 py-16 lg:grid-cols-[0.8fr_1.2fr]',
        className,
      )}
      aria-live="polite"
      aria-busy="true"
